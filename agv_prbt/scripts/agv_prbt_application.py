...
# 照片存放路径
cap_original_file_path = "/home/pilz/Pictures/agv_prbt/cap.png" 
cap_calibrated_file_path = "/home/pilz/Pictures/agv_prbt/cap_calibrated.png"
...
def start_program():
    ...
    while not rospy.is_shutdown():
        ...
        # 名片盒取料工序
        if box_request and (len(box_pick_Y_list) != 0):
        # if len(box_pick_Y_list) != 0:
            ...
            modbus_write(pss_modbus_write_dic['sucker_on'], [0])
            box_stock_pick_up_pose = Pose(position=Point(STOCK_BOX_X, box_pick_Y_list[0], STOCK_Z_UP), orientation=SUCKER_ORIENTATION)
            box_stock_pick_down_pose = Pose(position=Point(STOCK_BOX_X, box_pick_Y_list[0], STOCK_BOX_Z_DOWN), orientation=SUCKER_ORIENTATION)
            box_conveyor_place_up_pose = Pose(position=Point(0.399, 0.181, STOCK_Z_UP), orientation=SUCKER_ORIENTATION)
            box_conveyor_place_down_pose = Pose(position=Point(0.399, 0.181, PLATE_BOX_Z_DOWN), orientation=SUCKER_ORIENTATION)
        ...
        # 笔取料工序
        if pen_request and (len(pen_pick_Y_list) != 0):
        # if True and len(pen_pick_Y_list) != 0:
        ...
        # if True:
            ...
            modbus_write(pss_modbus_write_dic['use_sucker'], [1])
            modbus_write(pss_modbus_write_dic['sucker_on'], [0])
            box_conveyor_pick_up_pose = Pose(position=Point(-0.011, 0.33, STOCK_Z_UP), orientation=SUCKER_ORIENTATION)
            box_conveyor_pick_down_pose = Pose(position=Point(-0.011, 0.33, PLATE_BOX_Z_DOWN), orientation=SUCKER_ORIENTATION)
            ...
            box_handout_seq.append(Lin(goal=START_POSE, vel_scale=LIN_SCALE, acc_scale=0.1), blend_radius=0.1)
            box_handout_seq.append(Lin(goal=box_outlet_in_pose, reference_frame="prbt_base_link", vel_scale=LIN_SCALE, acc_scale=0.1), blend_radius=0.1)
        ...
        # if True:
            ...
            pen_handout_seq.append(Lin(goal=PEN_HANDOUT_MIDDLE_WP, vel_scale=LIN_SCALE, acc_scale=0.1), blend_radius=0.1)
            pen_handout_seq.append(Lin(goal=pen_outlet_in_pose, reference_frame="prbt_base_link", vel_scale=LIN_SCALE, acc_scale=0.1), blend_radius=0.1)
